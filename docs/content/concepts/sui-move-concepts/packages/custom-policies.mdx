...
You can change the current policy by calling one of the functions in `one::package` (`only_additive_upgrades`, `only_dep_upgrades`, `make_immutable`) on the package's `UpgradeCap` and a policy can become only more restrictive. For example, after you call `one::package::only_dep_upgrades` to restrict the policy to become additive, calling `one::package::only_additive_upgrades` on the `UpgradeCap` of the same package results in an error.
...
{@inject: crates/sui-framework/packages/one-framework/sources/package.move#fun=authorize_upgrade,commit_upgrade noComments}
...
{@inject: crates/sui-framework/packages/one-framework/sources/package.move#struct=UpgradeCap noComments}
...
{@inject: crates/sui-framework/packages/one-framework/sources/package.move#struct=UpgradeTicket noComments}
...
```
```sh
FETCHING GIT DEPENDENCY https://github.com/one-chain-labs/onechain.git
...
{@inject: crates/sui-framework/packages/one-framework/sources/package.move#struct=UpgradeReceipt noComments}
...
The signature of a custom `authorize_upgrade` can be different from the signature of `one::package::authorize_upgrade` as long as it returns an `UpgradeTicket`.

Finally, provide implementations of `commit_upgrade` and `make_immutable` that delegate to their respective functions in `one::package`:
...
Array [
    Object {
        ...
        "objectType": String("0x2::coin::Coin<0x2::oct::OCT>"),
        "objectId": String("<GAS>"),
        "version": String("12"),
