...
public fun withdraw(state: &mut State, account: &mut Account, ctx: &mut TxContext): Coin<OCT> {
...
public fun wrong_withdraw(state: &mut State, ctx: &mut TxContext): Coin<OCT> {
...
public fun new(deposit: Coin<OCT>, ctx: &mut TxContext): OwnedWallet {
    // Implementation omitted.
    abort(0)
}
...
public fun new_shared(deposit: Coin<OCT>, ctx: &mut TxContext) {
    // Implementation omitted.
