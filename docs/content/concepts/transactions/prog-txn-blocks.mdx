...
- The `commands` value is a vector of commands, `[Command]`. The possible commands are:
  ...
  - `SplitCoins` splits off multiple (one or more) coins from a single coin. It can be any `one::coin::Coin<_>` object.
  - `MergeCoins` merges multiple (one or more) coins into a single coin. Any `one::coin::Coin<_>` objects can be merged, as long as they are all of the same type.
  ...
  - `Upgrade` upgrades an existing package. The upgrade is gated by the `one::package::UpgradeCap` for that package.
...
If the object is owned by another object, as in it was sent to an object's ID via the `TransferObjects` command or the `one::transfer::transfer` function, then use `ObjectArg::Receiving(ObjectID, SequenceNumber, ObjectDigest)`. The object data is the same as for the `ImmOrOwnedObject` case.
...
- `SplitCoins`: produces (one or more) coins from a single coin. The type of each coin is `one::coin::Coin<T>` where the specific coin type `T` matches the coin being split.
- `Publish`: returns the upgrade capability, `one::package::UpgradeCap`, for the newly published package.
- `Upgrade`: returns the upgrade receipt, `one::package::UpgradeReceipt`, for the upgraded package.
...
- If the signature expects a `T`, the runtime checks the argument has type `T` and it is copied if `T: copy` and moved otherwise. No object in Sui has `copy` because the unique ID field `one::object::UID` present in all objects does not have the `copy` ability.
...
Object inputs have the type of their object `T` as you might expect. However, for `ObjectArg::Receiving` inputs, the object type `T` is instead wrapped as `one::transfer::Receiving<T>`. This is because the object is not owned by the sender, but instead by another object. And to prove ownership with that parent object, you call the `one::transfer::receive` function to remove the wrapper.
...
- The coin argument `CoinArg: Argument` must be a coin of type `one::coin::Coin<T>` where `T` is the type of the coin being split. It can be any coin type and is not limited to `SUI` coins.
...
- The result of the command is a vector of coins, `one::coin::Coin<T>`. The coin type `T` is the same as the coin being split, and the number of results matches the number of arguments
- For a rough signature expressed in Move, it is similar to a function `<T: key + store>(coin: &mut one::coin::Coin<T>, amounts: vector<u64>): vector<one::coin::Coin<T>>` where the result `vector` is guaranteed to have the same length as the `amounts` vector.
...
- The coin argument `CoinArg: Argument` must be a coin of type `one::coin::Coin<T>` where `T` is the type of the coin being merged. It can be any coin type and is not limited to `SUI` coins.
- The coin arguments `ToMergeArgs: [Argument]` must be `one::coin::Coin<T>` values where the `T` is the same type as the `CoinArg`.
- The coin argument `CoinArg` is taken by mutable reference.
- The merge arguments `ToMergeArgs` are taken by value (moved).
- The command does not produce any results (an empty result vector).
- For a rough signature expressed in Move, it is similar to a function `<T: key + store>(coin: &mut one::coin::Coin<T>, to_merge: vector<one::coin::Coin<T>>): ()`
...
The command produces a single result of type `one::package::UpgradeCap`, which is the upgrade capability for the newly published package.
...
The `UpgradeTicket: one::package::UpgradeTicket` is the upgrade ticket for the package being upgraded and is generated from the `one::package::UpgradeCap`. The ticket is taken by value (moved).

The command produces a single result type `one::package::UpgradeReceipt` which provides proof for that upgrade. For more details on upgrades, see [Upgrading Packages](../sui-move-concepts/packages/upgrade.mdx).
...
The inputs include the friend's address, the marketplace object, and the value for the coin split. For the commands, split off the coin, call the market place function, send the gas coin and one object, grab your address (via `one::tx_context::sender`), and then send the remaining object to yourself. For simplicity, the documentation refers to the package names by name, but note that in practice they are referenced by the package's Object ID.

To walk through this, first look at the memory locations, for the gas object, inputs, and results

```rust
Gas Coin: one::coin::Coin<OCT> { id: gas_coin, balance: one::balance::Balance<OCT> { value: 1_000_000u64 } }
Inputs: [
  Pure(/* @0x808 BCS bytes */ ...),
...
```rust
Gas Coin: Coin<OCT> { id: gas_coin, ... value: 500_000u64 ... } // The maximum gas is deducted
Inputs: [
  Pure(/* @0x808 BCS bytes */ ...),
...
```rust
Gas Coin: Coin<OCT> { id: gas_coin, ... value: 499_900u64 ... }
Inputs: [
  Pure(/* @0x808 BCS bytes */ ...),
  Marketplace { id: market_id, ... },
  Pure(/* 100u64 BCS bytes */ ...),
...
Results: [
  [Coin<OCT> { id: new_coin, value: 100u64 ... }], // The result of SplitCoins
],
```
...
Assume that `buy_two` deletes its `Coin<OCT>` object argument and transfers the `Balance<OCT>` into the `Marketplace` object.
...
Make another Move call, this one to `one::tx_context::sender` with the signature
