...
The function to add child objects to a parent might resemble the following. In this case, `ofield` is an alias for the `one::dynamic_object_field` package.
...
1. Use a `Publisher` object you own to set `one::display` for a specific type.
