...
Following these actions, the protocol computes the total amount of stake as the sum of staked OCT plus the storage fund.
...
- The total amount of stake rewards is distributed across various entities. The storage fund is taken into account in the calculation of the epoch total stake, which is not owned by any entities in the way that staked OCT is. Instead, the Sui economic model distributes the stake rewards accruing to the storage fund to validators for compensation of their storage costs.
...
The staking pool is implemented in a system-level smart contract ([staking_pool.move](https://github.com/MystenLabs/sui/blob/main/crates/sui-framework/packages/one-system/sources/staking_pool.move)) and is part of the Sui framework.
