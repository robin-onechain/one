...
| --- | --- | --- |
| Fields | Can store any value that has `store`, however an object stored in this kind of field is considered wrapped and is not accessible via its ID by external tools (explorers, wallets, and so on) accessing storage.| [`dynamic_field`](https://github.com/MystenLabs/sui/tree/main/crates/sui-framework/packages/one-framework/sources/dynamic_field.move) |
| Object field | Values must be objects (have the `key` ability, and `id: UID` as the first field), but are still accessible at their ID to external tools.| [`dynamic_object_field`](https://github.com/MystenLabs/sui/tree/main/crates/sui-framework/packages/one-framework/sources/dynamic_object_field.move) |
...
{@inject: crates/sui-framework/packages/one-framework/sources/dynamic_field.move#fun=add noComments}

**Dynamic object field**

{@inject: crates/sui-framework/packages/one-framework/sources/dynamic_object_field.move#fun=add noComments}
...
{@inject: crates/sui-framework/packages/one-framework/sources/dynamic_field.move#fun=borrow,borrow_mut noComments}

Where `object` is the UID of the object the field is defined on and `name` is the field's name.

:::info

`one::dynamic_object_field` has equivalent functions for object fields, but with the added constraint `Value: key + store`.
...
{@inject: crates/sui-framework/packages/one-framework/sources/dynamic_field.move#fun=remove noComments}
...
`one::dynamic_object_field` has an equivalent function for object fields.
