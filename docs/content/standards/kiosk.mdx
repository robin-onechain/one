...
To place an item to the kiosk, the owner needs to call the `one::kiosk::place` function on the `Kiosk` object and pass the `KioskOwnerCap` and the `Item` as arguments.
...
To take an item from a kiosk you must be the kiosk owner. As the owner, call the `one::kiosk::take` function on the `Kiosk` object, and pass the `KioskOwnerCap` and `ID` of the item as arguments.
...
To lock an asset in a kiosk, call the `one::kiosk::lock` function. To ensure that you can later unlock the asset you must associate a `TransferPolicy` with the asset.
