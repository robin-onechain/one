...
For consistency, Sui docs must use the following terms and phrases with the displayed capitalization (unless sentence structure determines otherwise):

{/***Full node** A node running on the Sui network. Written as Full node in a sentence.*/}

- **dApp:** A decentralized app that runs on a decentralized blockchain network.
- **Devnet:** A version of the Sui network for development and experimentation. 
- **Testnet:** A version of the Sui network for testing. 
- **Mainnet:** The Sui production network that processes public user transactions. 
- **Localnet:** A local Sui network. Define it parenthetically on first use in a topic: “a local Sui network (Localnet).”
- **Layer 1:** Layer 1 refers to a base network, such as Bitcoin, Ethereum, or Sui, and its underlying infrastructure. Layer 1 blockchains can validate and finalize transactions without the need for another network.
- **recovery passphrase (mnemonic):** The 12-word passphrase generated for a Sui address. The parenthetical is optional, but if used then subsequent uses should just use "mnemonic".
- **open source:** Use 2 words without a hyphen.
- **Sui:** Sui is a boundless platform to build rich and dynamic on-chain assets from gaming to finance.
- **SUI token:** The SUI token is the Sui platform's native asset.
- **SUI:** The unit of SUI tokens. For example, the list price for this NFT is 10 SUI.
...
- **Web2:** Web2 technically isn't a thing  but it looks better when in the same content as Web3. Here's how wikipedia defines Web 2.0: https://en.wikipedia.org/wiki/Web_2.0
- **key pair:** Two words.
