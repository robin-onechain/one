...
1.  Clone the Sui repository:
    ```sh
    $ git clone https://github.com/one-chain-labs/onechain.git -b branch-name
    ```
...
Sui supports the following API operations related to staking. You can find the source code in the [sui_system](https://github.com/MystenLabs/sui/blob/main/crates/sui-framework/packages/one-system/sources/sui_system.move) module.
...
public fun request_add_stake_mul_coin(
   self: &mut SuiSystemState,
   delegate_stakes: vector<Coin<OCT>>,
