...
```sh
$ cargo install --git https://github.com/one-chain-labs/onechain.git move-analyzer
```
...
```move
use one::{balance, coin::{Self, Coin}};
```
...
...
#[expected_failure(abort_code = one_system::validator_set::EInvalidCap)]
```
