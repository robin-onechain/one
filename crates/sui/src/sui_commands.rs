...
use crate::supper_committee::SuiSupperCommitteeCommand;
...
pub struct SuiEnvConfig {
    ...
    /// The OneChain environment to use. This must be present in the current config file.
...
pub enum SuiCommand {
    ...
    /// the OneChain config default dir or in the config_dir that was passed. If the default directory
    ...
    Start {
        ...
        /// one genesis -f --with-faucet generates a genesis config that can be used to start this
        /// proces. Use with caution as the `-f` flag will overwrite the existing config directory.
        /// We can use any config dir that is generated by the `one genesis`.
    ...
    /// Bootstrap and initialize a new one network
    #[clap(name = "genesis")]
    Genesis {
        ...
        #[clap(
            long,
            help = "Creates an extra faucet configuration for oneChain persisted runs."
        )]
        with_faucet: bool,
    ...
    /// Client for interacting with the OneChain network.
    #[clap(name = "client")]
    Client {
        #[clap(flatten)]
        config: SuiEnvConfig,
    ...
    #[clap(name = "supper-committee")]  ///add
    SupperCommittee {
        /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
        #[clap(long = "client.config")]
        config: Option<PathBuf>,
    ...
    /// Invoke OneChain's move-analyzer via CLI
        ...
        match self {
            SuiCommand::Network {
                ...
                let config: NetworkConfig = PersistedConfig::read(&config_path).map_err(|err| {
                    err.context(format!(
                        "Cannot open OneChain network config file at {:?}",
                        config_path
                    ))
                })?;
            ...
            SuiCommand::Move {
                ...
                match cmd {
                    sui_move::Command::Summary(mut s) if s.package_id.is_some() => {
                        let (_, client) = get_chain_id_and_client(
                            client_config,
                            "one move summary --package-id <object_id>",
                        )
                        .await?;
                        let Some(client) = client else {
                            bail!("`one move summary --package-id <object_id>` requires a configured network");
                        };
                        ...
                        if !s.summary.bytecode {
                            eprintln!("{}", 
                                "[warning] `one move summary --package-id <object_id>` only supports bytecode summaries. \
                    ...
                    sui_move::Command::Build(build) if build.dump_bytecode_as_base64 => {
                        // `one move build` does not ordinarily require a network connection.
                        ...
                        let (chain_id, client) = if build.ignore_chain {
                            ...
                            get_chain_id_and_client(
                                client_config,
                                "one move build --dump-bytecode-as-base64",
                            )
                            .await?
                        };
                ...
                if client_config.env.is_some() && matches!(cmd, sui_move::Command::Build(_)) {
                    let (chain_id, _) =
                        get_chain_id_and_client(client_config, "one move build").await?;
        ...
        let (network_config_path, sui_config_path) = match config {
                ...
                if committee_size.is_some() {
                    eprintln!(
                        ...
                        "[warning] The committee-size arg wil be ignored as a network \
                            ...
                            with the desired committee size. See `one genesis --help` for more \
                            information."
                            .yellow()
                            .bold()
                    );
            ...
            None => {
                ...
                if !network_config.exists() {
                    ...
                    .map_err(|_| {
                        anyhow!(
                            "Cannot run genesis with non-empty OneChain config directory: {}.\n\n\
                                ...
                                genesis, use one genesis --help to see how to generate a new \
                                genesis.",
                            sui_config.display(),
                        )
                    })?;
...
async fn genesis(
    ...
    let sui_config_dir = &match working_dir {
        ...
        // will not be created)
        Some(v) => v,
        // create default OneChain config dir if not specified
    ...
    // if OneChain config dir is not empty then either clean it
    // up (if --force/-f option was specified or report an
    // error
    let dir = sui_config_dir.read_dir().map_err(|err| {
        anyhow!(err).context(format!("Cannot open OneChain config dir {:?}", sui_config_dir))
    })?;
        ...
        if force {
            ...
            if is_compatible {
                ...
                fs::remove_dir_all(sui_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot remove OneChain config dir {:?}", sui_config_dir))
                })?;
                fs::create_dir(sui_config_dir).map_err(|err| {
                    anyhow!(err)
                        .context(format!("Cannot create OneChain config dir {:?}", sui_config_dir))
                })?;
            }
    ...
    if let Some(ssfn_info) = ssfn_info {
        for (i, ssfn) in ssfn_info.into_iter().enumerate() {
            ...
            let ssfn_config = FullnodeConfigBuilder::new()
                ...
                .with_db_path(PathBuf::from("/opt/one/db/authorities_db/full_node_db"))
                ...
                .with_genesis(Genesis::new_from_file("/opt/one/config/genesis.blob"))
                .build(&mut OsRng, &network_config);
    ...
    if !wallet_conf_path.exists() {
            ...
            None => {
                    ...
                    || matches!(read_line(), Ok(line) if line.trim().to_lowercase() == "y")
                {
                    let url = if accept_defaults {
                        ...
                        print!(
                            "OneChain Full node server URL (Defaults to OneChain Testnet if not specified) : "
                        );
                        read_line()?
                    };
        ...
        if let Some(env) = env {
            let keystore_path = match wallet_conf_path.parent() {
                ...
                Some(parent) => parent
                    .canonicalize()
                    .context("Could not find OneChain config directory")?,
