...
fun add_validator_with_min_voting_power() {
    ...
    assert!(validator_set.total_stake() == 10_000 * MIST_PER_OCT);
...
fun add_then_increase_stake_of_others() {
    ...
    num_validators.do!(|i| {
        let to_add = 10_000_000 * MIST_PER_OCT;
        let stake = validator_set.request_add_stake(
...
fun create_validator_with_initial_stake(
    ...
    let validator = validator::new_for_testing(
        ...
        vector[hint],
        if (initial_stake != 0) {
            option::some(balance::create_for_testing(initial_stake * MIST_PER_OCT))
        } else { option::none() },
        1,
        0,
        is_initial_validator,
