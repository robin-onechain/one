...
fun test_removed_value() {
    ...
    {
        let mut config: Config<WriteCap> = ts.take_shared_by_id(id);
        let removed_value = config.add_for_next_epoch<_, _, u8>(&mut WriteCap(), n, 0, ts.ctx());
    ...
    {
        let mut config: Config<WriteCap> = ts.take_shared_by_id(id);
        let removed_value = config.add_for_next_epoch<_, _, u8>(&mut WriteCap(), n, 1, ts.ctx());
        assert_eq!(removed_value, option::none());
        ts::return_shared(config);
    };
    ...
    {
        let mut config: Config<WriteCap> = ts.take_shared_by_id(id);
        let removed_value = config.add_for_next_epoch<_, _, u8>(&mut WriteCap(), n, 2, ts.ctx());
    ...
    {
        let mut config: Config<WriteCap> = ts.take_shared_by_id(id);
        let removed_value = config.remove_for_next_epoch<_, _, u8>(&mut WriteCap(), n, ts.ctx());
        assert_eq!(removed_value, option::none());
        let removed_value = config.add_for_next_epoch<_, _, u8>(&mut WriteCap(), n, 3, ts.ctx());
        assert_eq!(removed_value, option::none());
        ts::return_shared(config);
    };
    ...
    {
        let mut config: Config<WriteCap> = ts.take_shared_by_id(id);
        let removed_value = config.add_for_next_epoch<_, _, u8>(&mut WriteCap(), n, 4, ts.ctx());
